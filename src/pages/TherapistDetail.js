...
const StarRating = ({ score, max = 5, size = 'md', interactive = false, onSelect }) => {
...
const TherapistDetail = () => {
  ...
  const [ratingStatus, setRatingStatus] = useState(null); // { puedeCalificar, calificacionExistente }
  ...
  useEffect(() => {
    if (currentUser && therapist && !isOwnProfile) {
      getRatingStatus(therapist.id)
        ...
        .catch(() => setRatingStatus(null));
    }
  }, [therapist, currentUser]); // eslint-disable-line react-hooks/exhaustive-deps

  // Auto-seleccionar si hay exactamente una especialidad
  useEffect(() => {
  ...
  const handleSubmitRating = async () => {
    ...
    try {
      await createRating({
        ...
        score: ratingSelected,
        comment: ratingComment.trim() || null,
      });
      setRatingSuccess('¡Gracias por tu calificación!');
      setRatingStatus({ puedeCalificar: false, calificacionExistente: { score: ratingSelected, comment: ratingComment } });
      ...
      <main className="flex-1 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-16 w-full">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              {therapist.ratingCount > 0 && (
                <div className="mt-4">
                  <div className="flex items-center justify-center gap-2">
                    <StarRating score={Math.round(therapist.averageRating)} size="md" />
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {currentUser && !isOwnProfile && ratingStatus && (
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                <h2 className="text-lg font-semibold text-gray-900 mb-4">Calificación</h2>

                {ratingStatus.calificacionExistente ? (
                  <div>
                    <p className="text-sm text-gray-500 mb-2">Tu calificación:</p>
                    <div className="flex items-center gap-2">
                      <StarRating score={ratingStatus.calificacionExistente.score} size="md" />
                      <span className="text-sm font-medium text-gray-700">{ratingStatus.calificacionExistente.score}/5</span>
                    </div>
                    {ratingStatus.calificacionExistente.comment && (
                      <p className="mt-2 text-sm text-gray-500 italic">"{ratingStatus.calificacionExistente.comment}"</p>
                    )}
                    {ratingSuccess && <p className="mt-2 text-sm text-green-600 font-medium">{ratingSuccess}</p>}
                  </div>
                ) : ratingStatus.puedeCalificar ? (
                  <div>
                    ...
                    <div className="flex items-center gap-1 mb-4"
                      ...
                      {[1, 2, 3, 4, 5].map((star) => {
                        const activo = star <= (ratingHover || ratingSelected);
                        return (
                          <svg
                            key={star}
                      ...
                      {ratingSelected > 0 && (
                        <span className="ml-2 text-sm text-gray-500">
                          {['', 'Muy malo', 'Malo', 'Regular', 'Bueno', 'Excelente'][ratingSelected]}
                        </span>
                      )}
                    </div>
                    ...
                    <button
                      ...
                      disabled={!ratingSelected || submittingRating}
                      className={`mt-3 w-full py-2.5 rounded-xl text-sm font-semibold text-white transition-all ${ratingSelected && !submittingRating ? 'bg-gradient-to-r from-mystic-500 to-primary-600 hover:from-mystic-600 hover:to-primary-700' : 'bg-gray-200 text-gray-400 cursor-not-allowed'}`}
                    >
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
              ...
              {/* Bloquear calendario si hay especialidades y no se eligió ninguna */}
